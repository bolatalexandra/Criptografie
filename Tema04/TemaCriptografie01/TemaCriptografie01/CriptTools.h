...
template<typename MyInt>
long da_prim(MyInt min, MyInt max, MyInt nr) {
	int nrIncercari = 10;
	MyInt k;
...
public:
	...
	BigInt a_la_b_mod_c(BigInt a, BigInt b, BigInt c) {
		BigInt<N> p = 1;
		a = a % c;
		while (b > 0)
		{
	...
	BigInt<N> da_prim2(BigInt<N> min, BigInt<N> max, BigInt<N> nr) {
		int nrIncercari = 10;
		BigInt<N> k;
		if (min < 0 || max<0 || min>max)return-1;
		if (min == max)
	...
	BigInt da_cod(char c) {
		for (int i = 0; i < N; i++)
			if (caracter[i] == c)return i;
		return -1;
	}

	char da_caracter2(BigInt<N> cod) {
		return caracter[modulo2(cod, N)];
	}

	BigInt modulo2(BigInt<N> k, BigInt<N> n) {
		if (k < 0)k = n - (-k) % n;
		if (k >= n) return k % n;
		return k;
	}

	static BigInt<N> Sqrt(BigInt<N> a) {
		BigInt<N> x0 = a, x1 = (a + 1) / 2;
		while (x1 < x0) {
			x0 = x1;
			x1 = (x1 + a / x1) / 2;
		}
